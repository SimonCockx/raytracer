...
cornellCamera :: PerspectiveCamera
cornellCamera = createPerspectiveCamera 600 400 (Point 0 7 4) (Vector 0 0 (-1)) (Vector 0 1 0) (pi/2) (RegularGrid 16)

reflectiveCornell :: IO (Scene RGB)
reflectiveCornell = do
    ...
    return $ Scene (insertBoundingBoxes world) cornellCamera

diffuseCornell :: IO (Scene RGB)
diffuseCornell = do
    let floor = translate 0 (0::Double) 0 `transform` createBox 50 2 50
        left  = translate (-8:: Double) 0 0 `transform` createBox 2 50 50
        right = translate (8::Double) 0 0 `transform` createBox 2 50 50
...
camera1 = createPerspectiveCamera 600 400 (Point 0 0 0) (Vector 0 0 (-1)) (Vector 0 1 0) (pi/2) (RegularGrid 16)
...
vlekjeScene = do
  ...
  let floor = translate 0 (0 :: Double) 0 `transform` createBox 50 2 50
      ...
      teaPot1 =
        Transformed
          (translate (5 :: Double) 9 (-13) `transform` scaleUni (3 :: Double) `transform` rotateY (-pi / 6 :: Double))
          teaPotObj
      ...
      world =
        createWorld
          [ withMaterial teaPot1 (DiffuseTexture vlekje (\(Vector u v w) -> (u, v)))
...
textureMapScene = do
  ...
  let appleShape = translate 0 (-2) (-4 :: Double) `transform` appleObj
      ...
      strange =
        withMaterial sphere3 $
        ProceduralDiffuseTexture
          (\(Vector u v w) ->
             RGB
               (sin (10 * u * w) + 1 / (3 + acos v) ** 2)
               (abs v / (1 + cos (20 / (w + v ** 2)) ** 2))
               (exp (u + w - sqrt 2)))
      world = createWorld [apple, rainbowSphere1, rainbowSphere2, strange] [Light $ AmbientLight $ RGB 2.4 2.4 2.4]
  return $ Scene (insertBoundingBoxes world) camera1
...
tilesScene = do
  let size = 3
      ...
      tiles =
        map
          (\(x, z) ->
             (`withMaterial` Diffuse (fromIntegral (round (x + z) `mod` 2) *^ RGB 1 1 1)) $
             translate ((x - (xCount - 1) / 2) * size) (-1.005) ((-z * size) + size / 2) `transform`
             createBox size 1.0e-2 size)
          [(a, b) | a <- [0 .. (xCount - 1)], b <- [0 .. (zCount - 1)]]
      world = createWorld tiles [Light $ AmbientLight $ RGB 2.4 2.4 2.4]
  return $ Scene (insertBoundingBoxes world) camera1

softShadowScene :: IO (Scene RGB)
softShadowScene = do
  let floor = translate (-2) (-3) (-9 :: Double) `transform` createBox 20 2 20
      wall = translate (-0.5 :: Double) (-0) (-6.5) `transform` createBox 0.1 4 4
      light = translate 2.5 1 (-6.5 :: Double) `transform` createAreaLight (Vector 1 1 0) 4.5 4.5 (3 *^ white)
      world = createWorld [simpleObject wall, simpleObject floor, SceneLight light] [Light light]
  return $ Scene (insertBoundingBoxes world) camera1

sphereLineScene :: IO (Scene RGB)
sphereLineScene = do
  let n = 8
      spheres = map (\a -> translate (-0.9) (-0.9) (-a - 3 :: Double) `transform` createSphere 1) [0 .. (n - 1)]
      world =
        createWorld
          [simpleObject spheres]
...
shapeScene = do
  icosahedron <-
    Transformed (translate 4 1 (-9 :: Double) `transform` scaleUni 3) <$> readObjFile "objects/icosahedron.obj"
  let elipse =
        translate 0 (-2) (-10 :: Double) `transform` rotateZ (pi / 4 :: Double) `transform` scale (1 :: Double) 2 1 `transform`
        createSphere 1.5
      sphere = translate (-2) 0.5 (-11 :: Double) `transform` createSphere 1.5
      box = translate (-5) (-2) (-9 :: Double) `transform` createBox 0.5 1 5
      cylinder =
        translate 7 (-5) (-10 :: Double) `transform` rotateZ (pi / 18 :: Double) `transform`
        createCylinder (Vector 0 1 0) 1 3
      triangle = createTriangle (Point (-8) 0 (-8)) (Point (-3) 2 (-8)) (Point (-6) 5 (-8))
      world =
        createWorld
          [ simpleObject icosahedron
          , simpleObject elipse
          , simpleObject sphere
          , simpleObject box
...
phongScene = do
  ...
  let teaPot1 = Transformed (translate (-4 :: Double) (-2) (-9) `transform` scaleUni 3.5) teaPot
      ...
      world =
        createWorld
          [simpleObject teaPot1, simpleObject teaPot2]
...
lightningScene = do
  icosahedron <-
    Transformed (translate (4 :: Double) 1 (-10) `transform` scaleUni 3) <$> readObjFile "objects/icosahedron.obj"
  let sphere = translate (-2) 0.5 (-11 :: Double) `transform` createSphere 1.5
      ...
      world =
        createWorld
          [ withMaterial icosahedron $ Diffuse $ RGB 0.4 0 0.1
...
lonelyTeapot = do
  ...
  let floor = translate (-2) (-3) (-9 :: Double) `transform` createBox 20 2 20
      world =
        createWorld
          [withMaterial teaPot (Diffuse $ RGB 0.8 0.5 0.9), simpleObject floor]
          [ Light $ LongRangePointLight (Point (-2) 1 (-6)) (RGB 9 3 3)
          , Light $ LongRangePointLight (Point 0 3 (-9)) (RGB 3 9 3)
          ]
  return $ Scene (insertBoundingBoxes world) camera1
