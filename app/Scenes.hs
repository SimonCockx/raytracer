...
lightningScene = do
    ...
    let sphere   = translate (-2) 0.5 (-11::Double) `transform` createSphere 1.5
        ...
        world = World [ withMaterial icosahedron $ Diffuse $ RGB 0.4 0 0.1
...
lonelyTeapot = do
    ...
    let floor = translate (-2) (-3) (-9::Double) `transform` createBox 20 2 20
        ...
        world = World [ withMaterial teaPot (Diffuse $ RGB 0.8 0.5 0.9)
                      , simpleObject floor
                      ]
                      [ Light $ LongRangePointLight (Point (-2) 1 (-6)) (RGB 3 1 1)
                      , Light $ LongRangePointLight (Point 0 3 (-9)) (RGB 1 3 1)
...
siyunScene = do
    ...
    let floor = translate (-2) (-5) (-9::Double) `transform` createBox 20 2 20
        ...
        world = World [ withMaterial plane (Diffuse $ RGB 0 0.8 0)
                      , simpleObject floor
                      ]
