...
render :: (RayTracer r s, Show s) => WorkerStates Gen -> r -> Scene s -> IO Image
render gens tracer (Scene world camera) = computeImage gens $ rayTrace camera tracer world

gammaCorrectedRender :: (RayTracer r s, Show s) => WorkerStates Gen -> r -> Scene s -> IO Image
gammaCorrectedRender gens tracer (Scene world camera) = computeImage gens $ gammaCorrectImageM $ rayTrace camera tracer world
